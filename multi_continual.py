...
from reid.trainers import ContrastTrainer, PrompterTrainer
...
def main_worker(args):
    ...
    dataset_targets = [get_data(dataset_target, args.data_dir) 
                       for dataset_target in args.dataset_target]
    ...
    # Trainer
    trainer = ContrastTrainer(model, trans=args.trans, mean_std_groups=mean_std_groups)

    if args.replay:
        replay_memory, seen_ids = reservior_sampling([], dataset_source.train, seen_ids=0)

    #for stage in range(args.stages):
    for data_index, dataset_target in enumerate(dataset_targets):
        if args.dataset_target[data_index] == 'cuhksysu':
            iters = 200
            args.eps = 0.5
        elif args.dataset_target[data_index] == 'dukemtmc':
            iters = 200
            args.eps = 0.6
        elif args.dataset_target[data_index] == 'msmt17':
            iters = 400
            args.eps = 0.6
        # Optimizer
        ...
        prompter_new = Prompter().cuda()
        prompter_trainer = PrompterTrainer(prompter=prompter_new)

        print('======= Begining stage {} ======='.format(args.dataset_target[data_index]))
        for epoch in range(args.epochs):
            ...
            if args.replay:
                ...
                label_offset = 0
            for ((fname, _, cid), label) in zip(sorted(dataset_target.train), pseudo_labels):
                if label != -1:
            ...
            if args.replay and len(replay_memory) > 0:
                cluster_memory_loader = get_test_loader(dataset_target, args.height, args.width,
                                                        args.batch_size, args.workers, testset=sorted(replay_memory))
                memory_features, memory_labels = extract_features(model, cluster_memory_loader, print_freq=50)
                memory_features = torch.cat([memory_features[f].unsqueeze(0) for f, _, _ in sorted(replay_memory)], 0)
                memory_labels = torch.cat([memory_labels[f].unsqueeze(0) for f, _, _ in sorted(replay_memory)], 0).numpy()
            ...
            memory = ClusterMemory(model.module.num_features, num_cluster+label_offset, temp=args.temp,
                                   momentum=args.momentum, use_hard=args.use_hard).cuda()
            memory.features = F.normalize(cluster_features, dim=1).cuda()
            trainer.memory = memory

            train_loader = get_train_loader(dataset_target, args.height, args.width,
                                            args.batch_size, args.workers, args.num_instances, iters,
                                            trainset=pseudo_labeled_dataset)

            train_loader.new_epoch()

            if args.prompter and len(prompters) > 0:
                prompter = prompters[np.random.choice(len(prompters))]
            else:
                prompter = None
            ...
            if args.prompter:
                prompter_trainer.train(epoch, train_loader, train_iters=400)

            if (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1):
                mAP = evaluator.evaluate(test_loaders[data_index], dataset_target.query, dataset_target.gallery, cmc_flag=False)
                evaluator.evaluate(test_loader_source, dataset_source.query, dataset_source.gallery, cmc_flag=False)
        ...
        if args.prompter:
            prompter_new.eval()
            prompters.append(prompter_new)
        if args.replay:
            replay_memory, seen_ids = reservior_sampling(replay_memory, dataset_target.train,
                                                         seen_ids=seen_ids)
        ...
        save_checkpoint({
            ...
            'epoch': epoch + 1,
            'best_mAP': best_mAP,
        }, is_best=False, fpath=osp.join(args.logs_dir, 'checkpoint_{}.pth.tar'.format(args.dataset_target[data_index])))

    print('=== Performance Conclusion ===')
    for data_name in args.dataset_target:
        print('========== Stage {} ==========='.format(data_name))
        checkpoint = load_checkpoint(osp.join(args.logs_dir, 'checkpoint_{}.pth.tar'.format(data_name)))
        model.load_state_dict(checkpoint['state_dict'])
        print('======= Target dataset =======')
        for data_index, data_name in enumerate(args.dataset_target):
            print(data_name)
            dataset_target = dataset_targets[data_index]
            evaluator.evaluate(test_loaders[data_index], dataset_target.query, dataset_target.gallery, cmc_flag=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dt', '--dataset-target', nargs='+', default=['cuhksysu', 'msmt17'])
